...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height, prev_img, frame_num):
        """
        初始化
        ...
        self.prev_frame = utils.get_resized_image(prev_img, img_width, img_height)
        ...
        self.style_layer_w = [0.5, 1.0, 1.5, 2.0, 3.0]
    ...
    def losses(self):
        ...
        模型总体loss
        """
        with tf.compat.v1.variable_scope("losses", reuse=tf.AUTO_REUSE):
            # contents loss
...
if __name__ == "__main__":
    ...
    frame_array = []
    files = [f for f in os.listdir(pathIn) if isfile(join(pathIn, f)) if not f.startswith('.') if not f.startswith('resized')]
    ...
    for i in range(n):
        ...
        img_height = 300
        if i == 0:
            print('Stylizing %dth frame ...' % i)
            style_transfer = StyleTransfer(content_img, style_img, img_width, img_height, content_img, i)
            style_transfer.build()
            style_transfer.train(5)
            prev_img = content_img
            print('Done')
        else:
        # style transfer
            print('Stylizing %dth frame ...' % i)
            style_transfer = StyleTransfer(content_img, style_img, img_width, img_height, prev_img, i)
            style_transfer.build()
            style_transfer.train(5)
            prev_img = content_img
            print('Done')